...
class EditReply extends React.Component {
    ...
    fetchData (){
        let str = window.location.href.replace('edit','editReply');
        axios.post(str,'',{
            headers: {
    ...
    render() {
        return (
            <div>
                ...
                />
                <button onClick={this.EditReply} className="btn btn-success mt-2">Save</button>
            </div>
        );
    }
}

if(document.querySelector('#edit-reply')){
    ReactDOM.render(
        <EditReply />,
        document.querySelector('#edit-reply')
    );
}